...
{
    qDebug() << "on_start_drag!!!!!!!!!!!!!" << (drag->focus);
    doClearPointerFocus();

    if (drag->icon) {
        // TODO: should use new qtquick item follow cursor
        cursor->setSurface(QWSurface::from(drag->icon->surface), {0,0});
    }
    qDebug() << drag->started << " " << drag->cancelling << " ";
    auto *qdrag = QWDrag::from(drag);

    QObject::connect(qdrag, &QWDrag::focus, q_func(), [this,drag]() {
        qDebug() << "QWDrag on focus" << (drag->focus);
        if (!drag->focus)
            doClearPointerFocus();
    });

    QObject::connect(qdrag, &QWDrag::motion, q_func(), [](wlr_drag_motion_event *event) {
        qDebug() << "QWDrag on motion" << event->sx << " " << event->sy;
    });

    QObject::connect(qdrag, &QWDrag::drop, q_func(), [](wlr_drag_drop_event *event) {
        qDebug() << "QWDrag on drop" << (event->drag->focus);
    });

    QObject::connect(qdrag, &QWDrag::beforeDestroy, q_func(), [this]() {
        qDebug() << "QWDrag on beforeDestroy";
    });
}
...
void WSeat::notifyMotion(WCursor *cursor, WInputDevice *device, uint32_t timestamp)
{
 ...
 //qDebug() << "notifyMotion: " << focusWindow() << " " <<  handle()->handle()->pointer_state.focused_surface;
