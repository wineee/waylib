...
QuickForeignToplevelManagerAttached::QuickForeignToplevelManagerAttached(WSurface *target, QuickForeignToplevelManagerV1 *manager)
    ...
    , m_manager(manager)
{
    connect(manager, &QuickForeignToplevelManagerV1::requestActivate, this, [this](WToplevelSurface *surface, [[maybe_unused]] treeland_foreign_toplevel_handle_v1_activated_event *event) {
    ...
    connect(manager, &QuickForeignToplevelManagerV1::requestMinimize, this, [this](WToplevelSurface *surface, treeland_foreign_toplevel_handle_v1_minimized_event *event) {
        if (surface->surface() != m_target) {
            return;
        }
    ...
    connect(manager, &QuickForeignToplevelManagerV1::requestMaximize, this, [this](WToplevelSurface *surface, treeland_foreign_toplevel_handle_v1_maximized_event *event) {
        if (surface->surface() != m_target) {
            return;
        }
    ...
    connect(manager, &QuickForeignToplevelManagerV1::requestFullscreen, this, [this](WToplevelSurface *surface, treeland_foreign_toplevel_handle_v1_fullscreen_event *event) {
        if (surface->surface() != m_target) {
            return;
        }
    ...
    connect(manager, &QuickForeignToplevelManagerV1::requestClose, this, [this](WToplevelSurface *surface) {
        if (surface->surface() != m_target) {
            return;
        }
    ...
    connect(manager, &QuickForeignToplevelManagerV1::rectangleChanged, this, [this](WToplevelSurface *surface, treeland_foreign_toplevel_handle_v1_set_rectangle_event *event) {
        if (surface->surface() != m_target) {
            return;
        }
...
public:
    ...
    void initSurface(WToplevelSurface *surface) {
        ...
        connection.push_back(QObject::connect(surface, &WToplevelSurface::titleChanged, q_func(),
                         [=] { handle->setTitle(surface->title().toUtf8()); }));

        connection.push_back(QObject::connect(surface, &WToplevelSurface::appIdChanged, q_func(),
                         [=] { handle->setAppId(surface->appId().toUtf8()); }));

        connection.push_back(QObject::connect(surface, &WToplevelSurface::minimizeChanged, q_func(),
                         [=] { handle->setMinimized(surface->isMinimized()); }));

        connection.push_back(QObject::connect(surface, &WToplevelSurface::maximizeChanged, q_func(),
